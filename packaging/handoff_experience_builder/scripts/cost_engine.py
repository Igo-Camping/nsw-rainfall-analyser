...
OBSERVED_CONDITION_COL = "Observed_Condition"
SUBURB_COL = "Asset Suburb"
X_MID_COL = "XMid"
Y_MID_COL = "YMid"
CRITICALITY_COL = "Score"

# Proximity clustering options (metres)
PROXIMITY_OPTIONS = [250, 500, 1000, 2000, 3000]
MIN_RELINING_DIAMETER_MM = 300.0

ASSET_DIAM_CANDIDATES = [ASSET_DIAM_COL, "SWP_Pipe_Diameter_mm", "DiameterStr", "Diameter"]
ASSET_LEN_CANDIDATES = [ASSET_LEN_COL, "Spatial_Length_m", "Length_m"]
...
def derive_suburb_from_text(value) -> str:
    text = str(value or "").strip()
    if not text or text.upper() == "TBA":
        return ""
    nsw_match = re.search(r"\b([A-Z]+(?: [A-Z]+)*) NSW \d{4}\b", text)
    if nsw_match:
        return nsw_match.group(1).strip()
    trailing_match = re.search(r",\s*([A-Z]+(?: [A-Z]+)*)\s*$", text)
    if trailing_match:
        return trailing_match.group(1).strip()
    direct_upper = text.upper()
    if re.fullmatch(r"[A-Z]+(?: [A-Z]+)*", direct_upper):
        return direct_upper.strip()
    return ""


def is_probable_suburb_name(value: str) -> bool:
    text = str(value or "").strip()
    if not text:
        return False
    upper = text.upper()
    ...
    blocked_terms = [
        " ROAD", " STREET", " ST ", " AVENUE", " AVE", " CIRCUIT", " PARADE", " DRIVE", " CLOSE",
        " PLACE", " CRESCENT", " COURT", " WAY", " LANE", " RESERVE", " PARK", " OVAL", " WETLAND",
        " ROAD RESERVE", " PROPERTY", "\\", "/", "LOT ", "UNNAMED"
    ]
    if upper in blocked_exact:
        return False
    if any(term in upper for term in blocked_terms):
        return False
    return bool(re.fullmatch(r"[A-Z]+(?: [A-Z]+)*", upper))


def get_row_suburb(row, suburb_col: str | None = None) -> str:
    invalid_values = {"", "nan", "none", "null", "tba", "not applicable", "n/a"}
    for candidate in ["Formatted_Address", "Pipe_Start_Address", "Asset_Address"]:
        if candidate in row.index:
            derived = derive_suburb_from_text(row.get(candidate))
            if derived:
                return derived
    if suburb_col and suburb_col in row.index:
        direct = str(row.get(suburb_col) or "").strip()
        if direct and direct.lower() not in invalid_values:
            derived_direct = derive_suburb_from_text(direct)
            if derived_direct and is_probable_suburb_name(derived_direct):
                return derived_direct
    if "Physical_Location" in row.index:
        derived = derive_suburb_from_text(row.get("Physical_Location"))
        if derived and is_probable_suburb_name(derived):
            return derived
    return ""


def ensure_suburb_column(df: pd.DataFrame, suburb_col: str | None = None) -> tuple[pd.DataFrame, str]:
    out = df.copy()
    source_col = suburb_col or resolve_suburb_column(out)
    target_col = SUBURB_COL
    if target_col not in out.columns:
        out[target_col] = ""
    out[target_col] = out.apply(
        lambda row: get_row_suburb(row, suburb_col=source_col) or "TBA",
        axis=1,
    )
...
def normalize_asset_columns(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    diam_col = resolve_first_existing_column(out, ASSET_DIAM_CANDIDATES)
    len_col = resolve_first_existing_column(out, ASSET_LEN_CANDIDATES)

    if diam_col and diam_col != ASSET_DIAM_COL:
        out[ASSET_DIAM_COL] = out[diam_col]
    if len_col and len_col != ASSET_LEN_COL:
        out[ASSET_LEN_COL] = out[len_col]
    return out


def extract_condition_scores(df: pd.DataFrame) -> pd.Series:
    candidates = [
        OBSERVED_CONDITION_COL,
        CONDITION_COL,
        "Schedule7_Condition2",
        "Schedule7_Condition",
    ]
    for col in candidates:
        if col not in df.columns:
            continue
        raw = pd.to_numeric(df[col].astype(str).str.extract(r"(\d+)")[0], errors="coerce")
        if raw.notna().any():
            return raw
...
def coerce_diameter_value(value) -> float | None:
    ...
    if not text:
        return None
    direct = pd.to_numeric(pd.Series([text]), errors="coerce").iat[0]
    if pd.notna(direct):
        return float(direct)
    parsed = parse_diameter_from_descriptor(text)
...
def lookup_sp6_rate(diameter: float,
    ...
    target = coerce_diameter_value(diameter)
    length_value = pd.to_numeric(pd.Series([length_m]), errors="coerce").iat[0]
    if target is None or pd.isna(length_value):
        return None
    subset = df[
...
def calc_reconstruction_cost(
    ...
    breakdown.append({"item": f"Pipe ({pipe_class} {int(diameter_mm)}mm)", "unit": "m",
...
def split_streams(assets_df: pd.DataFrame):
    df = normalize_asset_columns(assets_df.copy())

    if CONDITION_COL not in df.columns and OBSERVED_CONDITION_COL not in df.columns:
        return df.iloc[0:0].copy(), df.iloc[0:0].copy(), df

    cond = extract_condition_scores(df)

    diam = pd.to_numeric(df[ASSET_DIAM_COL].apply(coerce_diameter_value), errors="coerce") if ASSET_DIAM_COL in df.columns else pd.Series(dtype=float, index=df.index)

    rel_df = df[(cond == 8) & (diam >= MIN_RELINING_DIAMETER_MM)].copy()
...
def _round_criticality(val) -> float:
    """Criticality is rounded to 1 decimal place."""
    return round(_safe_num(val, 0.0), 1)


def _condition_score(val) -> float:
    c = _safe_num(val, 0.0)
    return max(c, 0.0)


def _diameter_weight(val) -> float:
    d = _safe_num(val, 0.0)
    if d < 225:
        return 1.0
    elif d < 300:
...
def _build_package_priority_summary(df: pd.DataFrame) -> pd.DataFrame:
    grouped = df.groupby("package_id", dropna=False).agg(
        package_total_pipe_priority=("pipe_priority_score", "sum"),
        package_high_crit_count=("is_high_criticality", "sum"),
        package_large_diam_count=("is_large_diameter", "sum"),
        package_worst_condition=(CONDITION_COL, "max"),
        package_total_length=(ASSET_LEN_COL, "sum"),
        package_pipe_count=(ASSET_ID_COL, "count"),
    ).reset_index()

    grouped["package_priority_score"] = (
        grouped["package_total_pipe_priority"]
        + (grouped["package_high_crit_count"] * 5.0)
        + (grouped["package_large_diam_count"] * 3.0)
        + (pd.to_numeric(grouped["package_worst_condition"], errors="coerce").fillna(0.0) * 2.0)
    )
    ...
    grouped["package_priority_rank"] = np.arange(1, len(grouped) + 1)
    return grouped


def add_package_priority(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds pipe- and package-level priority fields using:
        - condition
        - diameter
        - criticality from the Score column

    Pipe score:
        condition_score * diameter_weight * (1 + criticality_1dp)

    Package score:
        sum(pipe scores)
        + 5 * high criticality pipes (>= 4.0)
        + 3 * large diameter pipes (>= 375mm)
        + 2 * worst condition in package
    """
    ...
    if CRITICALITY_COL not in out.columns:
        out[CRITICALITY_COL] = 0.0

    out[CRITICALITY_COL] = pd.to_numeric(out[CRITICALITY_COL], errors="coerce").fillna(0.0)
    out["criticality_1dp"] = out[CRITICALITY_COL].apply(_round_criticality)
    out["condition_score"] = pd.to_numeric(out[CONDITION_COL], errors="coerce").fillna(0.0).apply(_condition_score)
    out["diameter_weight"] = out[ASSET_DIAM_COL].apply(_diameter_weight)

    out["is_high_criticality"] = (out["criticality_1dp"] >= 4.0).astype(int)
    out["is_large_diameter"] = (pd.to_numeric(out[ASSET_DIAM_COL], errors="coerce").fillna(0.0) >= 375.0).astype(int)

    out["pipe_priority_score"] = (
        out["condition_score"]
        * out["diameter_weight"]
        * (1.0 + out["criticality_1dp"])
    )

    package_summary = _build_package_priority_summary(out)
    out = out.merge(package_summary, on="package_id", how="left")
    return out


def reindex_packages_by_priority(df: pd.DataFrame, prefix: str = "RLN_") -> pd.DataFrame:
    """
    Renumber package_id so the package number matches the priority rank.
    Example:
        RLN_001 = highest priority package
        RLN_002 = second-highest priority package

    The supplied prefix fully overrides any existing package_id prefix.
    """
    if df is None or df.empty:
        return df

    if "package_id" not in df.columns or "package_priority_rank" not in df.columns:
        return df.copy()

    out = df.copy()
    pkg_rank = (
        out[["package_id", "package_priority_rank"]]
        .drop_duplicates()
        .sort_values(["package_priority_rank", "package_id"], ascending=[True, True], kind="mergesort")
        .reset_index(drop=True)
    )

    mapping = {
        row["package_id"]: f"{prefix}{idx + 1:03d}"
        for idx, row in pkg_rank.iterrows()
    }

    out["package_id"] = out["package_id"].map(mapping).fillna(out["package_id"])
    return out


def sort_packages_by_priority(df: pd.DataFrame) -> pd.DataFrame:
    """Sort rows so highest-priority packages appear first."""
    if df is None or df.empty:
        return df

    out = df.copy()

    if "package_priority_rank" in out.columns:
        sort_cols = ["package_priority_rank", "package_id"]
        ascending = [True, True]
        if ASSET_ID_COL in out.columns:
    ...
    if "package_id" in out.columns:
        sort_cols = ["package_id"]
        if ASSET_ID_COL in out.columns:
            sort_cols.append(ASSET_ID_COL)
        return out.sort_values(sort_cols, ascending=True, kind="mergesort").reset_index(drop=True)
...
def summarise_pipe_packages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "pipe_cost" in df.columns:
        agg["pipe_cost"] = "sum"
    if "package_priority_rank" in df.columns:
        agg["package_priority_rank"] = "max"
    if "package_priority_score" in df.columns:
        agg["package_priority_score"] = "max"
    ...
    sort_cols = [c for c in ["package_priority_rank", "package_id"] if c in summary.columns]
    if sort_cols:
        summary = summary.sort_values(sort_cols, ascending=[True] * len(sort_cols), kind="mergesort").reset_index(drop=True)

    return summary


def package_diameters_text(pkg_df: pd.DataFrame) -> str:
    if ASSET_DIAM_COL not in pkg_df.columns or pkg_df.empty:
        return ""
    diams = []
    for value in pkg_df[ASSET_DIAM_COL].dropna().unique():
        parsed = coerce_diameter_value(value)
        if parsed is not None:
            diams.append(int(parsed))
    return ", ".join(f"⌀{d}" for d in sorted(set(diams)))


def package_max_distance_m(pkg_df: pd.DataFrame) -> float | None:
    if pkg_df.empty or X_MID_COL not in pkg_df.columns or Y_MID_COL not in pkg_df.columns:
        return None
    coords = pkg_df[[X_MID_COL, Y_MID_COL]].apply(pd.to_numeric, errors="coerce").dropna()
    if len(coords) < 2:
        return 0.0 if len(coords) == 1 else None
    values = coords.to_numpy(dtype=float)
    max_dist = 0.0
        ...
        if ASSET_DIAM_COL in pool.columns:
            pool = pool[
                pool[ASSET_DIAM_COL].apply(
                    lambda d: (
                        get_size_group(parsed_d)
                        if (parsed_d := coerce_diameter_value(d)) is not None
                        else None
                    ) in pkg_size_groups
                )
            ]
